...
#+ATTR_ORG: :width 500 
[[file:images/20210222130436-从零配置emacs教程/ivy-rich_的配置/20210222-133201_screenshot.png]]
...
f (telega-buffer-file-send) : 发送文档或者图片到聊天. 如果指定 `C-u`
则传输文件. 否则按照`iamge-mode major-mode`的方式, 将文件作为图片发送.
如果在交互模式下调用, 与当前buffer关联的文件会被当做FILE.
